...
makeMPS(SiteSet const& sites,
    ...
    for(auto b : range(N+1)) links.at(b) = Index(1,"l");
    for(auto j : range1(N))
        {
...
//    psi.Anc(1) *= setElt(links.at(0)(1));
    ...
    psi.setA(N,    psi(N)*setElt(links.at(N)(val(links.at(N)(1)))));
    
    psi.setA(1,psi(1)*setElt(links.at(0)(val(links.at(0)(1)))));
...
randImg(std::vector<ImgType> const& imgs,
    ...
    Error(tinyformat::format("Did not find image with requested label after %d tries",max_tries));
    return imgs.front();
    }
...
//        ITensor U,S,V;
//        if(1 < p && p < N) U = ITensor(findIndex(psi.A(p),Site),uniqueIndex(psi.A(p),psi.A(p+dp),Link));
//        else               U = ITensor(findIndex(psi.A(p),Site));
//        svd(AA,U,S,V,args);
//        psi.setA(p,U);
//        psi.setA(p+dp,S*V);
//        psi.Aref(p+dp) *= norm(AA)/norm(psi.A(p+dp));
//        if(!findIndex(psi.A(p+dp),Label)) Error("No Label index on new o.c.");
//        };
//    while(np > p)
//        {
//        moveBy(+1);
